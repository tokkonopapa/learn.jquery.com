...
-->
私たちにできる、そしてすべき改善は、プラグインのデフォルト設定を公開するコードにすることです。これはプラグインのユーザーがそれらを上書きし、最小限のコードを追加してプラグインをカスタマイズするのを容易にする上で、重要なことです。そこでまずは、関数オブジェクトを活用することから始めます。
...
// プラグインのデフォルト値 – プラグイン関数のプロパティとして追加する
$.fn.hilight.defaults = {
	foreground: "red",
	background: "yellow"
};
...
```
// 次の行は1度だけ呼び出せば十分で、何か "ready" といった
// ブロックの中から呼び出さなければならないということはない
$.fn.hilight.defaults.foreground = "blue";
```
...
-->
そして次のようにプラグインを呼び出せば、フォアグラウンド色に青を使えるようになります。
...
-->
この項目は前の項目と一緒で、プラグインを拡張する（そして他の人にプラグインを拡張してもらう）ための興味深い方法です。例えばプラグインがハイライトするテキストを整形する "format" という関数を定義を実装しているとします。次のように、ハイライトする関数の下にデフォルトの書式化メソッドの定義を実装することになるでしょう。
...
-->
オプション・オブジェクトの他に、コールバック関数でデフォルトの書式化をオーバーライドすることができる、もう1つのプロパティを簡単にサポートすることができます。それは、プラグインのカスタマイズをサポートするもう1つの優れた方法です。ここで示すテクニックは、再定義できるよう書式化関数を公開するという、さらに一歩進んだ手法です。このテクニックにより、他の人があなたのプラグインを独自にオーバーライドして出荷することができるようになります。言い換えれば、あなたのプラグイン様に他の人がプラグインを書けることを意味します。
...
-->
この記事で取り上げているとるに足らないプラグインの例だけでは、どのぐらい有用なのだろうと疑問に思うかもしれません。1つの実世界の例としては、[Cycle Plugin](http://malsup.com/jquery/cycle/) が挙げられます。このプラグインは、多数のビルトインされた切り替え効果、例えばスクロール、スライド、フェードなどをサポートしたスライドショーのプラグインです。しかし、誰かが望むかもしれないあらゆる切り替え効果を定義する現実的な方法はありません。そういう場合にこそ、この手の拡張性が役に立つのです。Cycle Plugin は "transitions" オブジェクトを公開することで、ユーザーが独自の切り替え効果の定義を追加することができるのです。このプラグインの定義は次のようになるでしょう。
...
-->
このテクニックにより、Cycle Plugin のための切り替え効果を定義したプラグインを出荷することができるのです。
...
-->
あなたのプラグインの一部を公開しオーバーライドさせるテクニックは非常に強力です。しかしどの実装部分を公開するかは慎重に考えなければなりません。一旦公開をすると、呼び出しの引数や意味を変更する際、後方互換が失われる可能性のあることを常に心に留めておく必要があります。一般論として、ある関数を公開するかどうか迷ったら、おそらく止めるべきです。
...
-->
それでは名前空間を汚すことなく、そして実装を公開することなく、更なる関数を定義するにはどうしたら良いでしょうか？ これはクロージャーの仕事です。例えば Firebug のコンソールに選択された要素の数をログするデバック関数を考えてみましょう。クロージャーを作るためには、（jQuery のオーサリング・ガイドラインに詳しくある通り）関数の定義全体を function で囲います。
...
-->
プラグインの種類によっては、[メタデータ・プラグイン](http://docs.jquery.com/Plugins/Metadata/metadata) がさらに強力な力を発揮します。個人的にはメタデータ・プラグインが好みです。なぜなら、プラグインのオプションをオーバーライドするのにとても慎み深いマークアップが可能となるからです（とりわけデモとか例題を作成する時に役に立ちます）。そしてそのサポートは非常に簡単なのです！
...
-->
ボブがすごいギャラリー・プラグイン（"superGallery" と呼びます）を作ったとしましょう。このプラグインは画像のリストを受け取り、ナビゲーションを作成します。ボブのプラグインはより興味よ引くよう幾つかのアニメーションを実行します。彼はできる限りカスタマイズが可能なプラグインを作ろうとしたので、次のように実装しました：
...
-->
このコードをみて最初に心に浮かぶのは（別に最初じゃなくても構わないですが）、このプラグインがあらゆるレベルのカスタマイズに答えるために、どれほどの融通を効かせなければならないかという見通しでしょう。実際にこのプラグインは、必要以上に大きなものとなるでしょう。人々が使ってみたいと思うのはせいぜい数キロバイト程度なのです。
...
-->
さて、我らが友人のボブはこれで良しとし、プラグインとそのカスタマイズのレベルにかなり満足しています。すべてのオプションはより汎用性のあるソリューションであり、それぞれがより多くの異なる状況で使えると信じています。
...
-->
さてもう1人の友人スーは、この新しいプラグインを使おうと思い立ちました。彼女は必要とされるオプションをすべて設定し、動作するソリューションを得ました。しばしプラグインを弄った後、5分経って彼女は考えました。それぞれの画像の幅がもっとゆっくりアニメーションすればずっと素敵なのに、と。そこで彼女はボブのドキュメントを探しましたが、*animateWidthDuration* というオプションは見つかりません！
...
-->
このプラグインがどれだけたくさんのオプションを持っているかは問題ではありません。どんなオプションを持つかなのです！
...
-->
ボブは少々行き過ぎでした。彼が提供したカスタマイズのレベルは、一見高そうに見えますが、実際には極めて低いものです。特にこのプラグインを使う時に制御したいと思うことを、考えうる限りの可能性をもって考慮した点です。ボブは特定のオプションをばかばかしいほど多く提供するというミスを犯したのです。その結果、彼のプラグインをカスタマイズすることが非常に困難になってしまったのです！
...
-->
ボブには新しいカスタマイズのモデルが必要なことは明らかです。制御性を失わず、必要な詳細を抽象化することです。
...
-->
ボブに高いレベルの簡略化が必要な理由は、jQuery のフレームワークはとてもこの考え方に適しているということです。オプション *previousButtonTextColor* を提供することは至ってシンプルですが、このプラグインのユーザーの大半は、制御の方法を必要としているのです！

<!--
Here are a few tips which should help you create a better set of customizable options for your plugins:
-->
次章以降では、よりよいカスタマイズのオプションを与えるプラグインの作り方に関するいくつかの秘訣を紹介します。
...
-->
プラグインを使う開発者は新しい言語や用語を学ぶ必要はなく、ただ使えればいいのです。
...
-->
ボブは *delay* オプションに最大限のカスタマイズ性を与えようと考えました。そこで彼は4つの異なる "quite short"、"very short"、"quite long"、"very long" という4つの異なる特別な遅延を定義しました。
...
-->
これは制御の幅を狭めるだけでなく、多くのスペースを必要とします。遅延時間を定義するのに12行のコードは多過ぎるとは思いませんか？ このオプションを設定するには遅延時間の合計を（ミリ秒の）数値で指定する方がいいでしょう。そうすればこれほどたくさんのスペースを取らずにオプションを処理できます。
...
-->
ここでのキーポイントは、抽象化によって制御のレベルを減らすことではありません。抽象化する時には、それが何であれ、思い切った単純化が必要です。それでもこのプラグインを使おうとする人にとっては、低レベルの制御（この場合の低レベルとは、抽象化していないことを指します）に勝るとも劣らない制御性を持っているのです！
...
// ただし明示的に深いコピーとするために、最初のパラメータを true を加えます。
var settings = $.extend( true, {}, defaults, options );
```
...
-->
*$.extend()* メソッドは、先の例のように引数を指定すれば、階層化されたオブジェクトすべてを再帰的に辿り、`default` と渡されたオプションの両方をマージしたバージョンを作ってくれます。
...
-->
コレによりプラグインのユーザーは、ラッパー要素に CSS のフックとなる任意の属性を与えられるようになり、クラスを追加したり ID 名を変更したりすることが容易になるばかりでなく、プラグインのソースを深いところまで読み込まなくても良いようになるのです。
...
-->
あなたのプラグインは、開発者が CSS を追加できる様な関連するスタイルシートがあるかもしれません。その場合でさえ、要素を得るためのセレクターを使わせることなく、JavaScript でスタイルを設定するのに便利な方法を提供することは良い考えです。
...
-->
*コールバックとは何でしょう？* – 基本的には、何かしらのイベントによって呼び出されれる関数のことです。通常は、コンポーネントの初期化関数（この場合、jQuery プラグイン）に引数として引き渡されます。
...
-->
もしあなたのプラグインが何らかのイベントによって起動されるなら、それぞれのイベントにコールバックする機能を提供することは良い考えです。加えて、プラグイン用のカスタムなイベントを作成し、それらにコールバックを与えることも可能です。ギャラリー・プラグインの例では、"onImageShow" コールバッックを追加するのが妥当でしょう。
...
-->
同様に "onImageHide" その他多数のコールバックを追加することができます。コールバックのポイントは、プラグインのユーザーがソースコードを読み込まなくとも機能を追加する簡単な方法を提供することです。
...
-->
あなたのプラグインは、どんな状況に対応できるようにすることがゴールではありません。逆に、制御するためのメソッドが少な過ぎても役に立たないでしょう。そう、常に妥協が必要なのです。常に次の3つを考慮すると良いでしょう：
...
- *サイズ*： プラグイン機能のレベルに照らして、サイズは適正か？ 非常にベーシックなツールチップを実装するプラグインが20Kバイトだったら、誰も使いません！
- *パフォーマンス*： オプションに必要以上に重たい処理を割いていないか？ スピードに影響を与えていないか？ そのオーバーヘッドはエンドユーザーに悪影響を引き起こしていないか？
